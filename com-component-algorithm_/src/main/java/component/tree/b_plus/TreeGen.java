...
public class TreeGen {
    ...
    public TreeGen(int m) {
        this._m = m;
        this._min = (int) Math.ceil((double)((double)_m/(double)2));//向上取整
    }

    public static TreeGen getCopyGen(TreeGen gen){
        TreeGen _gen = new TreeGen(gen.getM());
        List<Float> floats = gen.getAllKeyList();
        for (float f:floats){
            _gen.insert(f);
        }
    ...
    public void recursion_search_first_leaf(INode currNode){
        if (currNode == null){
            return;
        }
    ...
    public List<Float> getAllKeyList(){
        List<Float> floats = new ArrayList<>();
        TreeLeaf treeLeaf = getFirstLeaf();
        while (treeLeaf != null){
            for (float f:treeLeaf.keys){
                floats.add(f);
            }
            treeLeaf = treeLeaf.rightBrother;
        }
